...
class Board extends Component {
  constructor(props) {
    super(props);
    this.state = {
      board: Array(this.props.boardLength).fill(
        Array(this.props.boardWidth).fill(null)
      ),
    };
  }
  ...
  _isReachable(unit, col, row, placement = false) {
    ...
    if (placement) {
      ...
      const placementZone = this.props.placementZone;
      ...
      isReachable = playerNumber
        ? col > this.state.board[0].length - placementZone - 1
        : col < placementZone;
  ...
  _isForbidden(unit, col, row, placement = false) {
    ...
    if (placement) {
      ...
      isForbidden = playerNumber
        ? isForbidden || col <= this.state.board[0].length - placementZone - 1
  ...
  _isSelected(col, row, selectedUnit) {
    const unit =
      this.props.units[selectedUnit.playerNumber]?.[selectedUnit.unitNumber];
  ...
  renderSquare(col, row) {
    ...
    const containsUnits =
      containsUnits1[0] && containsUnits1[2]
        ? containsUnits1
        : containsUnits2[0] && containsUnits2[2]
        ? containsUnits2
        : null;
    ...
    if (!placement) {
      containsGhostUnits1 = this._containsUnits(
        this.props.futureUnits[0],
        col,
        row,
        0,
        placement,
      ...
      containsGhostUnits2 = this._containsUnits(
        this.props.futureUnits[1],
        col,
        row,
        1,
        placement,
      ...
      containsGhostUnits =
        containsGhostUnits2[0] && containsGhostUnits2[2]
          ? containsGhostUnits2
          : containsGhostUnits1[0] && containsGhostUnits1[2]
          ? containsGhostUnits1
          : null;
    }
    ...
    const containsPlayer =
      containsUnits2[0] || containsGhostUnits2?.[0]
        ? 1
        : containsUnits1[0] || containsGhostUnits1?.[0]
        ? 0
        : null;
